...
export default function Pricing() {
  return (
    <section className="py-24 md:py-32 px-6 relative overflow-hidden bg-white">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[500px] bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-cyan-200/20 via-transparent to-transparent -z-10"></div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 md:mb-24 space-y-4" data-aos="fade-up">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-50 border border-cyan-100 text-cyan-600">
            ...
            return (
              <div
                ...
                className={`relative flex flex-col rounded-[2.5rem] p-8 md:p-10 transition-all duration-500 group ${
                  isMiddle
                    ? "bg-white border-2 border-cyan-500 shadow-2xl shadow-cyan-500/20 scale-105 z-20"
                    : "bg-slate-50/50 border border-slate-200 hover:border-cyan-500/50"
                }`}
                data-aos="fade-up"
              ...
              >
                ...
                <div className="mb-8">
                  <h3 className={`text-xl font-bold mb-4 ${isMiddle ? "text-cyan-600" : "text-slate-900"}`}>
                    {tier.title}
                  </h3>
                ...
                <ul className="space-y-5 mb-10 flex-grow">
                  {tier.features.map((feat, fIdx) => (
                    <li key={fIdx} className="flex items-start gap-4 text-slate-700 group/item">
                      <div className={`mt-0.5 flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${
                        isMiddle ? "bg-cyan-500 text-white" : "bg-cyan-500/10 text-cyan-600"
                      }`}>
        ...
        <div className="mt-16 flex flex-wrap justify-center items-center gap-8 text-slate-400">
           ...
           <p className="text-sm font-medium text-slate-600">Need a custom quote? <a href="/contact" className="text-cyan-500 underline underline-offset-4">Talk to us</a></p>
        </div>
      </div>
    </section>
  );
