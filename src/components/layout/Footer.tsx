...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-white dark:bg-[#020617] border-t border-slate-200 dark:border-white/10 transition-colors duration-500 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 pb-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="flex gap-4">
              {[
                ...
                <Link key={i} href={social.href} className="p-3 rounded-full bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 hover:text-cyan-500 hover:bg-cyan-500/10 transition-all duration-300">
                  {social.icon}
                </Link>
              ))}
            </div>
          ...
          <div className="lg:col-span-3">
            <h4 className="font-bold text-slate-900 dark:text-white uppercase tracking-widest text-sm mb-6">Company</h4>
            <ul className="space-y-4">
              {["Work", "Services", "About", "Contact", "Careers"].map((item) => (
                <li key={item}>
                  <Link href={`/${item.toLowerCase()}`} className="group flex items-center gap-1 text-slate-600 dark:text-slate-400 hover:text-cyan-500 transition-colors">
          ...
          <div className="lg:col-span-4 bg-slate-100/50 dark:bg-white/5 p-8 rounded-3xl border border-slate-200/50 dark:border-white/5">
            <h4 className="font-bold text-slate-900 dark:text-white mb-6">Let&apos;s build something great.</h4>
            <div className="space-y-4 text-slate-600 dark:text-slate-400">
              <div className="flex items-start gap-3">
                <Mail size={18} className="mt-1 text-cyan-500" />
        ...
        <div className="mt-20 relative select-none pointer-events-none">
          <h1 className="text-center text-[08vw] font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 animate-gradient-x">
            ZEPHYRAA
          </h1>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-sm font-medium text-slate-500">
          ...
          <div className="flex gap-8">
            ...
            <Link href="/cookies" className="hover:text-cyan-500 transition">Cookies</Link>
          </div>
        </div>
      </div>
    </footer>
